...
export default function RecruitmentReportsScreen() {
  ...
  const [totals, setTotals] = useState<Omit<PerformanceRow, 'id' | 'team_id' | 'user_id' | 'report_date' | 'company_id' | 'total_call_duration'>>({
    monster: 0,
    dice: 0,
  ...
  const calculateTotals = useCallback(() => {
    const sum = (key: keyof PerformanceRow) => 
      ...
      <View style={styles.container}>
        ...
        <View style={styles.metricsContainer}>
          ...
          <View style={styles.metricsGrid}>
            {[
              { label: 'Monster', value: totals.monster, icon: 'paw-outline' },
              { label: 'Dice', value: totals.dice, icon: 'dice-outline' },
              { label: 'LinkedIn Views', value: totals.linkedin_profiles_viewed, icon: 'eye-outline' },
              { label: 'InMails', value: totals.linkedin_inmails_sent, icon: 'mail-outline' },
              { label: 'Calls', value: totals.total_calls, icon: 'call-outline' },
              { label: 'Submissions', value: totals.total_submissions, icon: 'document-text-outline' },
              { label: 'Interviews', value: totals.total_interviews, icon: 'people-outline' },
              { label: 'Offers', value: totals.offers, icon: 'gift-outline' },
              { label: 'Starts', value: totals.starts, icon: 'checkmark-circle-outline' },
            ].map((metric) => (
              <TouchableOpacity
                key={metric.label}
                style={[
